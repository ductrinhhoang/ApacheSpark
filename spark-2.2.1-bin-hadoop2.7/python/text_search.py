...
def main():
    ...
    if len(sys.argv) != 2:
    sys.stderr.write('Usage: {} <search_term>'.for
    ...
    matches = normalized_title.filter(lambda
    ted_movie.value in x[0])
    x:
    reques
    # Collect all the matching titles
    matching_titles = matches.map(lambda x: x[1]).distinct().col
    lect()
    # Display the result
